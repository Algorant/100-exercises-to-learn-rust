    ...
    loop {
        match receiver.recv() {
            ...
            Ok(Command::Update {
                ...
                let result = store.patch_ticket(patch);

                // Send response back - we ignore send errors since the client might have disconnected
                match result {
                    Ok(()) => {
                        let _ = response_channel.send(());
                    }
                    Err(_) => {
                        // For now, we still send () even if patch fails
                        // In a real app, you might want to return Result<(), PatchError>
                        let _ = response_channel.send(());
                    }
                }
            }
